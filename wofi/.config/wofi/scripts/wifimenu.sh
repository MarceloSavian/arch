...
CHENTRY=$(echo -e "$TOGGLE\nmanual\n$LIST" | uniq -u | wofi "$@" -i -d --style ~/.config/wofi/style.css --prompt "Wi-Fi SSID: ")
#echo "$CHENTRY"
...
if [ "$CHENTRY" = "manual" ] ; then
	...
	MSSID=$(echo "enter the SSID of the network (SSID,password)" | wofi "$@" -d --style ~/.config/wofi/style.css "Manual Entry: ")
	...
	if [[ $(echo "$KNOWNCON" | grep "$CHSSID") = "$CHSSID" ]]; then
		...
		if [[ "$CHENTRY" =~ "WPA2" ]] || [[ "$CHENTRY" =~ "WEP" ]]; then
			WIFIPASS=$(echo "if connection is stored, hit enter" | wofi "$@" -P -d --style ~/.config/wofi/style.css --prompt "password")
		fi
